...
uint8_t Adafruit_Fingerprint::getModel(uint8_t *buffer,
                                       uint16_t bufferSize,
                                       uint16_t *received)
{
    *received = 0;

    // Enviar comando UploadChar
    uint8_t data[] = { FINGERPRINT_UPLOAD, 0x01 };
    ...
    // Recibir todos los paquetes de datos
    while (true)
    {
        if (getStructuredPacket(&packet) != FINGERPRINT_OK)
            return FINGERPRINT_PACKETRECIEVEERR;
        ...
        // packet.length incluye los 2 bytes del checksum
        if (packet.length < 2)
            return FINGERPRINT_BADPACKET;

        uint16_t payloadSize = packet.length - 2;

        if ((*received + payloadSize) > bufferSize)
            return FINGERPRINT_BADPACKET;

        memcpy(buffer + *received, packet.data, payloadSize);
...
/************************************************************************* */
//CUSTOM IMPLEMENTATION TO ADD UPLOADING BUFFER SUPPORT

uint8_t Adafruit_Fingerprint::uploadModel(
    ...
    uint8_t slot)
{
    ...
    uint16_t payload = packet_len;

    while (length > 0)
    {
        uint16_t n = min(length, payload);
        ...
        Adafruit_Fingerprint_Packet p(type, n, (uint8_t*)buffer);
