...
struct Adafruit_Fingerprint_Packet {
  ...
  */
  ...
  Adafruit_Fingerprint_Packet(uint8_t type, uint16_t length, uint8_t *data) {
    ...
    address[2] = 0xFF;
    address[3] = 0xFF;
    memcpy(this->data, data, min<uint16_t>(length, sizeof(this->data)));
  }
...
public:
  ...
  uint8_t getModel(uint8_t *buffer, uint16_t bufferSize, uint16_t *received);
  uint8_t uploadModel(const uint8_t *buffer, uint16_t length,
                    uint8_t slot = 1);
